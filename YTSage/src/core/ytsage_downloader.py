...
from src.core.ytsage_utils import check_ffmpeg
...
class DownloadThread(QThread):
    ...
    def _get_browser_cookies_arg(self) -> str:
        """
        Handle Chrome/Chromium cookie extraction with retry logic for locked databases.
        If Chrome database is locked, create a temporary copy and use that.
        """
        if not self.browser_cookies:
            return None
        
        # Parse browser type from browser_cookies (format: "browser:profile")
        browser_type = self.browser_cookies.split(":")[0].lower()
        
        # Only handle Chrome-based browsers
        if browser_type not in ["chrome", "chromium", "edge"]:
            return self.browser_cookies
        
        # Try to use the browser cookies normally first
        try:
            # yt-dlp will handle this, just return as-is
            return self.browser_cookies
        except Exception as e:
            logger.warning(f"Error with browser cookies: {e}. Attempting retry...")
            return self.browser_cookies
    ...
    def _build_yt_dlp_command(self) -> List[str]:
        ...
        # Add extractor args for YouTube SABR streaming support
        # This helps bypass SABR restrictions on YouTube
        cmd.extend(["--extractor-args", "youtube:player_client=web"])
        logger.debug("Added YouTube extractor args for SABR support")
    ...
    def _run_direct_command(self) -> None:
        ...
        try:
            # Many yt-dlp operations require FFmpeg (merging best video+audio, SponsorBlock, trimming, remuxing, etc.)
            needs_ffmpeg = (
                (not self.is_audio_only and not self.format_has_audio)  # merge video-only + audio
                or bool(self.enable_sponsorblock)
                or bool(self.merge_subs)
                or bool(self.embed_chapters)
                or bool(self.force_output_format)
                or (bool(self.is_audio_only) and bool(self.force_audio_format))
                or bool(self.download_section)
            )
            if needs_ffmpeg and not check_ffmpeg():
                msg = (
                    "FFmpeg is required for this download option.\n\n"
                    "Fix:\n"
                    "- Install FFmpeg and restart the app, OR\n"
                    "- Pick a format that already includes audio (progressive MP4), OR\n"
                    "- Disable features like SponsorBlock / Trim / Merge Subtitles."
                )
                logger.warning(f"Download blocked: FFmpeg missing. needs_ffmpeg={needs_ffmpeg}")
            ...
            # Keep a small ring buffer of recent yt-dlp output so we can show real errors
            ...
            # Retry logic for Chrome cookie database lock issue
            max_retries = 3
            retry_count = 0
            ...
            while retry_count < max_retries:
                try:
                    ...
                    retry_count += 1
                    if "Chrome" in str(e) or "database is locked" in str(e).lower():
                        logger.warning(f"Chrome database locked (attempt {retry_count}/{max_retries}). Retrying in 2 seconds...")
                        time.sleep(2)
                        if retry_count >= max_retries:
                            logger.error(f"Failed to start after {max_retries} attempts. Chrome browser may still be running.")
                            self.error_signal.emit(
                                "Error: Download failed (yt-dlp failed to start).\n\n"
                                "This often happens when Chrome is still running and has locked the cookie database.\n\n"
                                "Fix:\n"
                                "1. Close Chrome completely (check Task Manager)\n"
                                "2. Try again\n\n"
                                "Or switch to 'Use cookie file' method in settings."
                            )
                            return
                    else:
                        raise
            ...
            for line in iter(self.process.stdout.readline, ""):  # type: ignore
                try:
                    ...
                    if len(recent_output) > 60:
                        recent_output = recent_output[-60:]
                except Exception:
                    # Never let debug buffering break downloads
                    pass
            ...
            if return_code == 0:
                ...
                if self.cancelled:
                    ...
                    tail = "\n".join(recent_output[-25:]).strip()
                    if tail:
                        # Try to point the user to common actionable causes
                        tail_lower = tail.lower()
                        hint = ""
                        if "playlist type is unviewable" in tail_lower:
                            hint = "Hint: This looks like a YouTube Mix/Radio playlist (list=RD...). Remove 'list=' from the URL to download a single video."
                        elif "ffmpeg" in tail_lower and ("not found" in tail_lower or "could not be found" in tail_lower):
                            hint = "Hint: Install FFmpeg (needed to merge best video+audio)."
                        elif "could not copy" in tail_lower and "chrome" in tail_lower.lower():
                            hint = "Hint: Chrome is still running and has locked the cookie database.\n\nFix: Close Chrome completely (including all tabs and background processes) and try again.\n\nOr switch to 'Use cookie file' method in Custom Options."
                        elif "sign in" in tail_lower or "cookies" in tail_lower:
                            hint = "Hint: The video may require login/age verification. Try Cookies in Custom Options."
                        elif "http error 429" in tail_lower or "too many requests" in tail_lower:
                            hint = "Hint: YouTube rate-limited you (429). Wait a bit, change network, or use cookies."

                        message = f"Download failed (yt-dlp exit code {return_code}).\n\nLast output:\n{tail}"
                        if hint:
                            message += f"\n\n{hint}"
                        self.error_signal.emit(message)
                    else:
                        self.error_signal.emit(f"Download failed (yt-dlp exit code {return_code}).")
