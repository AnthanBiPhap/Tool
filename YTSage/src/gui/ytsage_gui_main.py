...
class YTSageApp(QMainWindow, FormatTableMixin, VideoInfoMixin):  # Inherit from mixins
    ...
    def _analyze_url_with_subprocess(self, url) -> None:
        ...
        try:
            ...
            # If user pasted a watch URL that belongs to a playlist, we prefer analyzing as playlist first
            # but some "Mix/Radio" playlists (often list starts with RD) are unviewable via yt-dlp.
            playlist_url = None
            video_only_url = None
            if video_id and playlist_id and parsed.path.endswith("/watch"):
                playlist_url = f"https://www.youtube.com/playlist?list={playlist_id}"
                video_only_url = f"https://www.youtube.com/watch?v={video_id}"

            # Build command for basic info extraction
            def run_dumpjson(target_url: str):
                cmd = [yt_dlp_path, "--dump-json", "--no-warnings", target_url]
                
                # For channels/playlists, use --flat-playlist for faster extraction
                # This extracts video list quickly without full format info for each video
                if any(pattern in target_url.lower() for pattern in ['/channel/', '/c/', '/user/', '/playlist']):
                    cmd.insert(2, "--flat-playlist")
                    logger.debug("Using --flat-playlist for faster channel/playlist extraction")
                ...
                # Increased timeout to 180s (3 minutes) for large channels/playlists
                return subprocess.run(cmd, capture_output=True, text=True, timeout=180, creationflags=SUBPROCESS_CREATIONFLAGS)

            # Prefer playlist URL when applicable (gives playlist selector UI),
            # but gracefully fall back to single-video if YouTube marks it unviewable.
            primary_url = playlist_url or raw_url
            result = run_dumpjson(primary_url)

            if result.returncode != 0:
                logger.error(f"yt-dlp failed: {result.stderr}")
                stderr_lower = (result.stderr or "").lower()

                if video_only_url and "playlist type is unviewable" in stderr_lower:
                    logger.warning(
                        "Playlist is unviewable via yt-dlp; retrying analysis as a single video (dropping list=...)"
                    )
                    result = run_dumpjson(video_only_url)
                    if result.returncode != 0:
                        logger.error(f"yt-dlp failed after fallback: {result.stderr}")
            ...
            if first_info.get("_type") == "playlist" or len(json_lines) > 1:
                ...
                # If first video doesn't have format info (due to --extract-flat), fetch it now
                if "formats" not in self.video_info and self.playlist_entries[0].get("id"):
                    logger.debug("Extracting format info for first playlist video...")
                    first_video_url = f"https://www.youtube.com/watch?v={self.playlist_entries[0]['id']}"
                    try:
                        format_cmd = [yt_dlp_path, "--dump-json", "--no-warnings", first_video_url]
                        if self.cookie_file_path:
                            format_cmd.extend(["--cookies", str(self.cookie_file_path)])
                        elif self.browser_cookies_option:
                            format_cmd.extend(["--cookies-from-browser", self.browser_cookies_option])
                        
                        format_result = subprocess.run(format_cmd, capture_output=True, text=True, timeout=30, creationflags=SUBPROCESS_CREATIONFLAGS)
                        if format_result.returncode == 0:
                            format_info = json.loads(format_result.stdout.strip())
                            # Copy format info to first playlist entry
                            self.video_info["formats"] = format_info.get("formats", [])
                            logger.debug(f"Extracted {len(self.video_info.get('formats', []))} formats for playlist")
                    except Exception as e:
                        logger.warning(f"Failed to extract format info for first video: {e}")
