...
class AutoUpdateThread(QThread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            # If yt-dlp is missing or version is not parseable, skip auto-update cleanly
            if (
                not current_version
                or current_version.strip().lower() in {"not found", "unknown", "n/a"}
                or "error" in current_version.lower()
            ):
