"""
This module defines centralized constants used across the MikoYT application.
...
if OS_NAME == "Windows":
    ...
    APP_DIR: Path = Path(os.environ.get("LOCALAPPDATA", USER_HOME_DIR / "AppData" / "Local")) / "MikoYT"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "mikoyt_config.json"
    APP_HISTORY_FILE: Path = APP_DATA_DIR / "mikoyt_history.json"
    ...
    # Always use user data directory for app data, logs, config, and binaries
    # Even when frozen, we don't want to create these folders next to the executable
    APP_DIR: Path = USER_HOME_DIR / "Library" / "Application Support" / "MikoYT"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "mikoyt_config.json"
    ...
    # Always use user data directory for app data, logs, config, and binaries
    # Even when frozen, we don't want to create these folders next to the executable
    APP_DIR: Path = USER_HOME_DIR / ".local" / "share" / "MikoYT"
    APP_BIN_DIR: Path = APP_DIR / "bin"
    APP_DATA_DIR: Path = APP_DIR / "data"
    APP_LOG_DIR: Path = APP_DIR / "logs"
    APP_CONFIG_FILE: Path = APP_DATA_DIR / "mikoyt_config.json"
